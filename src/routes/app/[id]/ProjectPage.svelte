<script lang="ts">
    ...
    let { currentModule: project, childModules, childModulesError, handleTitleUpdate, openCardPopover, cardPopoverId, showCreateNewPopover = $bindable(), createNewTrigger = $bindable(), displayMode} = $props();
    ...
    <section bind:this={section} style="{isRowLayout ? "width" : "height"}: {sectionSize}px;">
        ...
        />
	    {#key project.id}
        	<TiptapEditor id={project.id} content={project.body} />
		{/key}
    </section>
